...
const AnimatedButton = ({
  ...
  // Базовые премиальные стили: чистый трекинг, жесткие контролируемые тайминги (cubic-bezier)
  const baseStyles = `
  ...
  // Сетка дизайн-системы для вариаций (адаптировано под светлые и темные фоны)
  const variants = {
    // 1. Глубокий темный (основной)
    ...
    // 2. Чистый светлый (для темных подложек / контраста)
    ...
    // 3. Акцентный фирменный (если есть контрастный цвет бренда)
  ...
  // Динамические цвета для инверсивных анимаций (переливы и заливки)
  const isDarkVariant = ['dark', 'accent'].includes(variant);

  // Подстраиваем цвет блика/заливки под тип кнопки, чтобы его ВСЕГДА было видно
  ...
  return (
    <Element
      ...
      {...props}
    >
      {/* ---------------- ВАРИАНТЫ АНИМАЦИЙ (ВЫБИРАЮТСЯ ЧЕРЕЗ PROPS) ---------------- */}

      {/* 1. SHIMMER: Элегантный лазерный блик (адаптивный) */}
      ...
      {/* 2. FILL: Деликатное заполнение объема всей кнопки */}
      ...
      {/* 3. LIFT: Интерактивная внутренняя глубина */}
      ...
      {/* СПЕЦИАЛЬНЫЙ ХОВЕР-ЭФФЕКТ ДЛЯ ВАРИАНТА ТЕКСТОВОЙ КНОПКИ (Умная линия) */}
      {variant === 'text' && (
